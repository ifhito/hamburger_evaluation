...
`design/files/hamburger-evaluation-redesign.penpot` は、**新しい見た目の案(ラフ)**です。いまの画面(`hamburger-evaluation.penpot`)とは別のファイルで、こちらは上書きしていません。話し合いで決めた方向(黒を基調・ハンバーガーの写真が主役・マスタードの黄色を 1 色だけ・現代的なゴシック・日本語と英語の切り替え・PC とモバイルを同じくらい)を、最初の 3 画面で確かめるためのものです。色や大きさの値は「案」で、見て調整します。
...
| ページ | 中身 |
|---|---|
| `値` | 色 11 色、文字と背景のコントラストの実測、文字のスタイル、キーボード操作の表示の見本 |
| `画面(PC)` | 幅 1280px の画面 4 枚 |
| `画面(モバイル)` | 幅 375px の画面 4 枚 |
...
- **ショップの一覧**: 写真をポスターのように大きく見せ、写真の下側に、暗いグラデーションを敷いて、店名と平均評価・件数を重ねます。PC は 3 列、モバイルは 1 列です。審査待ちの店には「審査待ち」の札、レビューがまだない店には「まだレビューがありません」と出します
- **レビューの一覧**: 1 件ずつのカードで、上に写真、下に星・日付・コメント・バーガーとお店の名前・投稿した人を並べます。PC は 2 列、モバイルは 1 列です。右下の「詳しく見る →」で詳細に進みます。写真のない投稿は、写真の場所に灰色の面と「写真なし」を出しています(見せ方は、あとで決めます)
- **レビューの詳細**: 上から順に、大きな写真(バーガー名とお店の名前を重ねる)→ 平均評価と件数(「レビューを書く」ボタンつき)→ レビューを新しい順に縦に並べます。PC もモバイルも同じ流れです。開いているレビューは、黄色の枠と「表示中のレビュー」の札で分かるようにし、自分の投稿には「編集」「削除」を出します
- **英語**: ヘッダーの `JA` / `EN` で切り替える想定です。英語のショップの一覧を PC とモバイルで 1 枚ずつ作り、文字が長くなっても収まることを確かめています(店名は投稿された文字のままで、翻訳しません)
- **ロゴ**: 文字だけです。`Hamburger`(太字・白)と `Evaluation`(標準・黄色)を、同じ Noto Sans JP で組んでいます。イラストは使いません
- **星とボタンの見分け**: 星は文字の中に黄色で置き、ボタンは黄色の面に黒い文字で描きます(形と場所で見分けます)。空の星は灰色です
...
| 名前 | 値 | 使う所 |
|---|---|---|
| `background` | `#0a0a0a` | ページの背景 |
| `surface` | `#141414` | カード・入力欄の背景 |
| `surface-2` | `#1e1e1e` | 選択中のカード・言語切替の選択中 |
| `border` | `#2e2e2e` | 区切り線・カードの枠(飾り) |
| `control-border` | `#6e6e6e` | 入力欄・ボタンの枠(部品の境界) |
| `text` | `#f4f4f2` | 本文・見出し |
| `text-muted` | `#a6a6a2` | 補足の文字(日付・件数) |
| `text-on-photo-muted` | `#d8d8d4` | 写真の上の補足の文字 |
| `accent` | `#e9b824` | 差し色(マスタード黄)。星・押せるボタン・リンク |
| `on-accent` | `#0a0a0a` | 黄色い面の上の文字 |
| `star-empty` | `#6e6e6e` | 空の星 |

- 黄色は、ボタン・星・リンクだけに使います。削除やエラーの色(赤など)は、まだ決めていません(いまの案は、削除ボタンも、灰色の枠のボタンです)
- ライト(明るい背景)は作りません
...
`値` ページの表と同じです。ページの中で、実際の色の値から計算しています(`design/redesign/tokens.html`)。図形(星・枠)は 3:1、文字は 4.5:1 が基準です。写真の上の文字は、写真がまっ白でも、暗い帯(黒の不透明度 0.8 以上)が重なるので、いちばん悪い場合の背景の色で測っています。

| 組み合わせ(文字 / 背景) | 比率 | 基準 | 結果 |
|---|---|---|---|
| 本文の文字 / ページの背景 | 17.98:1 | 4.5:1 以上 | ○ |
| 本文の文字 / カードの背景 | 16.73:1 | 4.5:1 以上 | ○ |
| 補足の文字 / ページの背景 | 8.11:1 | 4.5:1 以上 | ○ |
| 補足の文字 / カードの背景 | 7.54:1 | 4.5:1 以上 | ○ |
| 補足の文字 / 選択中のカードの背景 | 6.83:1 | 4.5:1 以上 | ○ |
| 黄色の文字・リンク / ページの背景 | 10.70:1 | 4.5:1 以上 | ○ |
| 黄色の文字・リンク / カードの背景 | 9.95:1 | 4.5:1 以上 | ○ |
| 黄色の文字・リンク / 選択中のカードの背景 | 9.01:1 | 4.5:1 以上 | ○ |
| ボタンの文字(黒) / 黄色の面 | 10.70:1 | 4.5:1 以上 | ○ |
| 星(黄) / ページの背景(図形は 3:1) | 10.70:1 | 3:1 以上 | ○ |
| 星(空) / ページの背景(図形は 3:1) | 3.88:1 | 3:1 以上 | ○ |
| 入力欄・ボタンの枠 / ページの背景(図形は 3:1) | 3.88:1 | 3:1 以上 | ○ |
| 写真の上: 白い文字 / 暗い帯(最悪ケース #333333) | 11.47:1 | 4.5:1 以上 | ○ |
| 写真の上: 補足の文字 / 暗い帯(最悪ケース #333333) | 8.84:1 | 4.5:1 以上 | ○ |
| 写真の上: 黄色の星 / 暗い帯(最悪ケース #333333) | 6.83:1 | 4.5:1 以上 | ○ |
| 写真の上: 「審査待ち」の札の文字 / 札(最悪ケース #474747) | 8.44:1 | 4.5:1 以上 | ○ |

押せる部分は 44px 以上にしています(ボタン・入力欄・タブ・言語の切り替え・「詳しく見る →」などの行)。キーボードで選んだときは、黄色の 2px の枠を、要素の 2px 外側に出す想定です(`値` ページに見本)。

### 写真・文言・数字は、仮のものです

- **写真は、まだありません。** 写真の場所には、暗い茶色や緑のグラデーションと「写真」の文字を置いています。実際の写真が入ると、見た目の印象が変わるので、写真を入れた確認が、あとで必要です
...
「いまの画面のデザイン」と同じく、ヘッドレスのブラウザで HTML を開いて、位置・色・文字を一覧(JSON)にし、Penpot の API で登録しています(文字は編集できる文字のままです)。見本の HTML は `design/redesign/` にあります(`mock.css` が見た目、`shops.html`・`reviews.html`・`review-detail.html`・`tokens.html` が画面。言語は `?lang=en` で切り替え)。値や配置を直したいときは、この HTML を直して、次の手順で作り直します。リポジトリのルートで実行します。
...
# 1. 見本の HTML を、位置・色・文字の一覧(JSON)にする(8 画面と、値のページ。色の値とコントラストの計算結果も出す)
...
- `CLIP=1` にすると、`render_penpot.js` が、画面の枠だけを切り出した JPEG を作ります(既定の PNG は、枠のまわりの余白つきです)
- 見本の HTML(ブラウザ)と Penpot の画面を画像にして、主な文字 12 か所の上下の位置を、画素で比べました。ずれは、どこも 1px 以内でした。Noto Sans JP では、Penpot が文字を、登録した行の位置(y)よりも大きさの 0.28 倍ほど上に描いたので、行の位置を「行の上端 + 行の高さ」(`build_redesign.py` の `BASELINE` = 1.0)で登録しています。「いまの画面から作り直す」の `build_penpot.py` の既定は、以前のとおり 0.8 のままです(Penpot の画像で文字が少し上に見えるのは、同じ理由と考えられますが、そのフォントでは確かめていません)
- 長方形の位置と大きさは、整数の px に丸めています(小数のままだと、暗い帯(グラデーション)の端に、細い線が出るため)
- `.penpot` の大きさは、約 0.79 MB(787,338 バイト)です。画像は 9 枚で、1 枚あたり 35〜180 KB です

### できていないこと・注意(このラフ)

- **部品(コンポーネント)にしていません。** ボタン・カード・札などは、同じ見た目の普通の図形です。決まった方向で画面を増やすときに、部品にします
- **状態は、画面 1 種類ずつです。** マウスを乗せた・押した・入力中・エラー・空・読み込み中などは、作っていません(キーボード操作の表示だけ、`値` ページに見本があります)
- **配置は、位置の固定です。** Penpot の自動配置(Auto layout)や、大きさの追従は付けていません
- **写真の暗い帯の濃さ**は、実際の写真の明るさで見え方が変わります。写真を入れた確認が、必要です
