...
function render() {
 ...
 const section = ['shops','burgers','reviews'].includes(route)?'search':route;
 ...
 document.querySelectorAll('.desktop-links a,.about-link,.profile-button,.record-button').forEach(a=>{a.removeAttribute('aria-current');if(a.getAttribute('href')==='#'+route)a.setAttribute('aria-current','page');});
 if (route==='home') page.innerHTML=`<section class="hero"><p class="eyebrow">BURGER STACK</p><h1>次の一口に、<br>出会おう。</h1><p>探して、食べて、記録する。<br>あなたの好きが、誰かのきっかけに。</p><a class="button" href="#shops">お店を探す <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M 4 12 L 20 12 M 14 6 L 20 12 L 14 18"/></svg></a></section><section><div class="section-heading"><h2>みんなの食べた記録</h2><a href="#reviews">すべて見る <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M 4 12 L 20 12 M 14 6 L 20 12 L 14 18"/></svg></a></div>${cards()}</section>`;
 ...
 else if(route==='about') page.innerHTML=`<section class="hero"><p class="eyebrow">Burger Stackについて</p><h1>世の中に、もっと<br>ハンバーガーを！！！</h1><p>お気に入りの一口を探して、<br>ハンバーガーの記録を積み重ねよう。</p></section><section class="empty"><h2>好きな一口を、分かち合う。</h2><p>紹介ページへの入口は、どの画面でもヘッダーに。ここからサービスの思いや使い方に出会えます。</p><a class="button" href="#shops">お店を探す</a></section>`;
 ...
 document.title = `${titles[route] || ({home:'ホーム',record:'記録する',profile:'マイページ',about:'Burger Stackについて',ai:'AIから使う'}[route])} — Burger Stack デザイン案`;
}
