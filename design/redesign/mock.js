...
(() => {
  ...
  const out = document.body.dataset.auth === 'out'; // サインインしていない人のヘッダー(サインイン・新規登録が並ぶ)
  const header = `<header class="site-header"><div class="wrap bar">
    ...
    <nav class="tabs">${tab('shops', 'ショップ', 'Shops')}${tab('reviews', 'レビュー', 'Reviews')}${out ? tab('signin', 'サインイン', 'Sign in') + tab('signup', '新規登録', 'Sign up') : '<a class="tab" href="#">alice</a>' + tab('', 'サインアウト', 'Sign out')}</nav>
