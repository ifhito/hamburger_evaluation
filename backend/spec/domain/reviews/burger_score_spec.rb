...
RSpec.describe Reviews::BurgerScore do
  describe "#initialize" do
    it "rounds weighted_average to 2 decimal places" do
      score = Reviews::BurgerScore.new(weighted_average: 3.14159, confidence: 0.8, sample_size: 5)
      expect(score.weighted_average).to eq(3.14)
    end

    it "clamps confidence between 0.0 and 1.0" do
      score = Reviews::BurgerScore.new(weighted_average: 4.0, confidence: 1.5, sample_size: 10)
      expect(score.confidence).to eq(1.0)
    end

    it "is frozen after initialization" do
      score = Reviews::BurgerScore.new(weighted_average: 4.0, confidence: 0.8, sample_size: 5)
  ...
  describe "#reliable?" do
    it "returns true when confidence >= 0.6" do
      score = Reviews::BurgerScore.new(weighted_average: 4.0, confidence: 0.6, sample_size: 10)
      expect(score.reliable?).to be true
    end

    it "returns false when confidence < 0.6" do
      score = Reviews::BurgerScore.new(weighted_average: 4.0, confidence: 0.3, sample_size: 1)
  ...
  describe ".empty" do
    it "returns a BurgerScore with zero values" do
      score = Reviews::BurgerScore.empty
