...
-- メールアドレスは、大文字小文字を区別せずに一意(「Alice@…」と「alice@…」を別のアカウントにしない)。全行が
-- 対象なので、退会済みのメールも再利用しない。事前の確認(check-then-act)だけでは、並行する登録で破られるので、
-- DB が保証する。上の UNIQUE(email)は、メールでの検索(ログイン)の索引としても使う。
