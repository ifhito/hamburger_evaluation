...
func TestOAuthRoutes(t *testing.T) {
	newRouter := func(oauth handler.OAuthEndpoints) http.Handler {
		if oauth == nil {
			return handler.NewRouter(okPinger, nil, unusedSignups(), nil, nil, nil, nil, nil, nil, nil)
		}
		return handler.NewRouter(okPinger, nil, unusedSignups(), nil, nil, nil, nil, nil, &handler.OAuth{Endpoints: oauth}, nil)
	}
