...
// API を登録する。nil なら、これらは未登録で、404 になる。
func NewRouter(db Pinger, auth *usecase.Auth, signups *usecase.Signups, shops *usecase.Shops, reviews *usecase.Reviews, users *usecase.Users, photoFiles http.Handler, oauth *OAuth) http.Handler {
	mux := http.NewServeMux()
	if photoFiles != nil {
		mux.Handle("GET /photos/", http.StripPrefix("/photos/", photoFiles))
	}
	registerRoutes(mux, append(oauthRoutes(oauth, auth), []route{
