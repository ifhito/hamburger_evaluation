...
func TestReviewRepositoryCreateReviewForNamedBurger(t *testing.T) {
	...
	t.Run("ショップの中に同じ名前のバーガーが複数あるとき、作成が最も古いものが選ばれ、作成が同時刻なら id の小さいものが選ばれる", func(t *testing.T) {
		...
		if _, burger := mustNamedCreate(t, shopC, "Tie"); burger.ID != smaller {
...
func TestReviewRepositoryBurgerStats(t *testing.T) {
	...
	burger := dbtest.InsertUUIDRow(ctx, t, conn, insertBurger, "Stats Burger")
	...
	t.Run("AC4 discard 済みの user の review と履歴は除外される", func(t *testing.T) {
		ac4Burger := dbtest.InsertUUIDRow(ctx, t, conn, insertBurger, "AC4 Burger")
	...
	t.Run("AC3 1 つの burger への並行する create で更新が失われない", func(t *testing.T) {
		...
		// である。
		for i := 0; i < 5; i++ {
			raceBurger := dbtest.InsertUUIDRow(ctx, t, conn, insertBurger, fmt.Sprintf("Race Burger %d", i))
	...
	t.Run("失敗した書き込みは burger_stats に手を付けない", func(t *testing.T) {
		errBurger := dbtest.InsertUUIDRow(ctx, t, conn, insertBurger, "Error Burger")
