...
// admin の viewer であっても、他人の Email と Admin は決して入らない。
func TestUserProfileFor(t *testing.T) {
	...
	selfView := func(u domain.User) domain.UserProfile {
		return domain.UserProfile{ID: u.ID, Username: u.Username, Email: ptr(u.Email), Admin: ptr(u.Admin)}
	}
	...
	tests := []struct {
		...
		{name: "admin でない本人の admin は nil ではなく false のポインタで返す", target: target, viewer: &target, want: domain.UserProfile{ID: uid.N(1), Username: "alice", Email: ptr("alice@example.com"), Admin: ptr(false)}},
