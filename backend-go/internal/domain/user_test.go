...
func TestUserProfileFor(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "admin でない本人の admin は nil ではなく false のポインタで返す", target: target, viewer: &target, want: domain.UserProfile{ID: 1, Username: "alice", Email: ptr("alice@example.com"), Admin: ptr(false)}},
