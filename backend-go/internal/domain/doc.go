...
// 集約に属さない規則は、規則ごとのファイルに置く（page.go: 一覧のページ送り、photo.go: 写真の大きさの上限、
// credentials.go・password.go・email.go など: 入力の検証）。usecase・handler・adapter は、その規則を呼ぶだけで、
// 件数・上限・丸め・文字数などの数値や判定を、自分では持たない。1 ファイルの中は
...
//     Reviews、Users、SignupVerifications、MailDeliveries、OAuthGrants、OAuthTokenSessions）に置く。Service は作らない。
...
//     統計の保存。退会 → 影響する burger の一覧の読み取り → 各 burger の再計算の依頼の登録。確認リンクでの登録
//     = 確認待ちのロック → 確認待ちの読み取り → ユーザーの作成 → 確認待ちの削除）は、repository だけを持つ
