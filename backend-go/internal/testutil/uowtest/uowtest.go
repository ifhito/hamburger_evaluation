...
// ShopStats は、domain.ShopStatRepository(ショップの集計のロック・保存・再計算の依頼)と usecase.ShopStatsQuery
// (集計の元データの読み取りと、再計算の依頼の一覧)の、両方を兼ねる代役である(Stats のショップ版)。呼び出しを、
// Ops に次の形で呼ばれた順に記録する。
//
//	"lock:<ショップ ID>"                ショップの行のロック
//	"facts:<ショップ ID>"               集計の元データの読み取り
//	"save:<ショップ ID>"                集計の保存
//	"burger-shops:<バーガー ID>"        バーガーが紐づくショップの一覧の読み取り
//	"request:<ショップ ID>"             再計算の依頼の登録
//	"complete:<ショップ ID>@<version>"   再計算を終えた依頼の削除
//	"failure:<ショップ ID>@<version>"    再計算の失敗の記録
//	"due"                               再計算の時期が来ている依頼の一覧の読み取り
//
// Facts、BurgerShops、Due が未設定(nil)のときは、元データもショップも依頼も空として返す。
type ShopStats struct {
	mu sync.Mutex
	// Ops は、上の形式で記録した操作の並びである(呼ばれた順)。
	Ops []string
	// Saved は、保存された集計である(保存された順)。
	Saved []domain.ShopStat
	// Facts は ListShopReviewFacts、BurgerShops は ListBurgerShopIDs、Due は ListDueShopRecalcRequests の振る舞いである。
	Facts       func(ctx context.Context, shopID string) ([]domain.ShopReviewFact, error)
	BurgerShops func(ctx context.Context, burgerID string) ([]string, error)
	Due         func(ctx context.Context, now time.Time, maxAttempts, batch int) ([]domain.ShopRecalcRequest, error)
	// Failures は、記録された再計算の失敗である(順序つき)。
	Failures []RecordedShopFailure
	// LockErr と SaveErr を設定すると、それぞれロックと保存がそのエラーで失敗する。
	LockErr, SaveErr error
	// RequestErr は再計算の依頼の登録、CompleteErr は依頼の削除、FailureErr は失敗の記録のエラーである。
	RequestErr, CompleteErr, FailureErr error
...
// RecordedShopFailure は、ショップの集計の再計算の失敗として記録された内容である。
type RecordedShopFailure struct {
	ShopID  string
	Version int64
	Failure domain.RecalcFailure
}
...
var (
	_ domain.ShopStatRepository = (*ShopStats)(nil)
	_ usecase.ShopStatsQuery    = (*ShopStats)(nil)
)

func (s *ShopStats) record(op string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.Ops = append(s.Ops, op)
}

// LockShopStat は、ロックの呼び出しを記録する(LockErr があればそのエラーで失敗する)。
func (s *ShopStats) LockShopStat(_ context.Context, shopID string) error {
	s.record("lock:" + shopID)
	return s.LockErr
}

// UpdateShopStat は、保存の呼び出しと、保存された集計を記録する(SaveErr があればそのエラーで失敗し、集計は記録しない)。
func (s *ShopStats) UpdateShopStat(_ context.Context, stat domain.ShopStat) error {
	s.record("save:" + stat.ShopID)
	if s.SaveErr != nil {
		return s.SaveErr
	}
	s.mu.Lock()
...
// CreateShopStatRecalcRequest は、再計算の依頼の登録を記録する。
func (s *ShopStats) CreateShopStatRecalcRequest(_ context.Context, shopID string) error {
	s.record("request:" + shopID)
	return s.RequestErr
}

// DiscardShopStatRecalcRequest は、依頼の削除を記録する。CompleteRejected なら、消せなかった(false)を返す。
func (s *ShopStats) DiscardShopStatRecalcRequest(_ context.Context, shopID string, version int64) (bool, error) {
	s.record(fmt.Sprintf("complete:%s@%d", shopID, version))
	if s.CompleteErr != nil {
		return false, s.CompleteErr
	}
	return !s.CompleteRejected, nil
...
// UpdateShopStatRecalcFailure は、再計算の失敗の記録を保存する。
func (s *ShopStats) UpdateShopStatRecalcFailure(_ context.Context, shopID string, version int64, failure domain.RecalcFailure) (bool, error) {
	s.record(fmt.Sprintf("failure:%s@%d", shopID, version))
	if s.FailureErr != nil {
		return false, s.FailureErr
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	s.Failures = append(s.Failures, RecordedShopFailure{ShopID: shopID, Version: version, Failure: failure})
	return true, nil
}

// ListShopReviewFacts は、Facts があればその結果を、なければ空を返す。
func (s *ShopStats) ListShopReviewFacts(ctx context.Context, shopID string) ([]domain.ShopReviewFact, error) {
	s.record("facts:" + shopID)
	if s.Facts == nil {
		return nil, nil
	}
	return s.Facts(ctx, shopID)
}

// ListBurgerShopIDs は、BurgerShops があればその結果を、なければ空を返す。
func (s *ShopStats) ListBurgerShopIDs(ctx context.Context, burgerID string) ([]string, error) {
	s.record("burger-shops:" + burgerID)
	if s.BurgerShops == nil {
		return nil, nil
	}
	return s.BurgerShops(ctx, burgerID)
}

// ListDueShopRecalcRequests は、Due があればそれを返し、なければ空を返す。
func (s *ShopStats) ListDueShopRecalcRequests(ctx context.Context, now time.Time, maxAttempts, batch int) ([]domain.ShopRecalcRequest, error) {
	s.record("due")
	if s.Due == nil {
		return nil, nil
	}
...
type UoW struct {
	...
	Stats   *Stats
	// ShopStats は、ショップの集計に対する操作の代役である。未設定(nil)なら、Do が空のものを作る。
	ShopStats *ShopStats
...
func (u *UoW) Do(ctx context.Context, fn func(ctx context.Context, tx usecase.Tx) error) error {
	...
	tx := usecase.Tx{
		...
		Stats:          u.Stats,
		ShopStats:      domain.NewShopStats(u.ShopStats),
		ShopStatsReads: u.ShopStats,
