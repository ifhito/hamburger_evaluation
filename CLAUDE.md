...
- 上限(すべて backend で判定する): ファイルは 5 MiB、寸法は 1 辺 10,000 px かつ 2,400 万画素(**HEIC は 1,600 万画素**。HEIC のデコードは 1 画素あたり約 31 バイトのメモリを使い、1,600 万画素で約 490 MiB、2,400 万画素では約 760 MiB になって、コンテナの上限(1 GiB)に収まらないため)。寸法は、デコードの前に、ヘッダーから読んで確かめる。HEIC のデコードは同時に 1 件(JPEG などと合わせて全体で 2 件)、8 秒で時間切れ
