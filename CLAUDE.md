...
- **エラーの文言の言語**(S51 で段階的に対応中。#136): リクエストの `Accept-Language` で、日本語(`ja`)か英語(`en`)を選ぶ(q 値・`ja-JP`・`*` に従う。どちらでもない・ヘッダーなしは英語 = いままでの文言の契約)。言語を選ぶのは handler(`langOf`)だけで、domain は、文言を「キー + 引数」(`domain.Message`)で返し、言語ごとの書式のカタログ(`domain/messages.go`。英語・日本語の両方が必須で、値の並びも同じ。構造テストが強制する)を持つだけである。**いま日本語になるのは、検証の失敗の 422(`domain.ValidationError` 由来の `{"errors":[…]}`)だけ**で、そのほかの 4xx(見つからない・権限・認証・入力の形・写真・Google・OAuth の許可の画面・MCP のツール)は、対応が済むまで英語のままである(済んだら、この項を更新する)。OAuth の RFC のプロトコルの識別子(`error` の値など)と、5xx の `internal server error` は、対応が済んでも英語のまま。
