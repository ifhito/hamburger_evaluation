...
- **エラーの文言の言語**: 利用者に見える 4xx の文言は、リクエストの `Accept-Language` で、日本語(`ja`)か英語(`en`)を選ぶ(q 値に従う。どちらでもない・ヘッダーなしは英語 = いままでの文言の契約)。言語を選ぶのは handler(`langOf`)だけで、domain は、文言を「キー + 引数」(`domain.Message`)で返し、言語ごとの書式のカタログ(`domain/messages.go`。英語・日本語の両方が必須で、値の並びも同じ。構造テストが強制する)を持つだけである。検証の 422(`{"errors":[…]}`)は、この仕組みで日本語になる。`error` などのプロトコルの識別子(OAuth の RFC のもの)と、5xx の `internal server error` は、言語によらず英語のまま。
