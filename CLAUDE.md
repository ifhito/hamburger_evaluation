...
- **エラーの文言の言語**(段階的に対応中): リクエストの `Accept-Language` で、日本語(`ja`)か英語(`en`)を選ぶ(q 値・`ja-JP`・`*` に従う。どちらでもない・ヘッダーなしは英語 = いままでの文言の契約)。言語を選ぶのは handler(`langOf`)だけで、文言は「キー + 引数」で表し、言語ごとの書式のカタログ(業務の規則の文言は `domain/messages.go` の `domain.Message`、handler が返す 4xx の文言は `adapter/handler/messages.go` の `apiMessage`。型を分けている。英語・日本語の両方が必須で、値の並びも同じ。構造テスト(`internal/testutil/msgcheck`)が強制する)で文字列にする。本文の作り方は、`writeError`・`writeErrorList`・`writeValidation`(言語に従う。`Vary: Accept-Language` も付く)と、`writeInternalError`(5xx。英語の固定)だけで、文字列を直接書いて本文を作ることは、構造テストが禁じる(言語の切り替えをすり抜けるため)。**いま日本語になるのは、検証の失敗の 422、見つからない・権限・認証(サインインの失敗・確認トークン)・リクエストの形・写真・クエリの誤りの 4xx、`/mcp` の入り口の 401・403** で、Google のサインインの案内・OAuth の許可の画面・MCP のツールの中のエラーは、対応が済むまで英語のままである(済んだら、この項を更新する)。次は、対応が済んでも英語のままである: OAuth の RFC のプロトコルの識別子(`error` の値など)、5xx の `internal server error`・`database unavailable`、写真の配信(`GET /photos/…`)の 404(本文が text/plain で、API のエラーではない)。
