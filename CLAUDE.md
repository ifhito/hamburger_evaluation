...
- **エラーの文言の言語**(S51 で段階的に対応中。#136): リクエストの `Accept-Language` で、日本語(`ja`)か英語(`en`)を選ぶ(q 値・`ja-JP`・`*` に従う。どちらでもない・ヘッダーなしは英語 = いままでの文言の契約)。言語を選ぶのは handler(`langOf`)だけで、文言は「キー + 引数」(`domain.Message`)で表し、言語ごとの書式のカタログ(業務の規則の文言は `domain/messages.go`、handler が返す 4xx の文言は `adapter/handler/messages.go`。英語・日本語の両方が必須で、値の並びも同じ。構造テスト(`internal/testutil/msgcheck`)が強制する)で文字列にする。handler の本文は、`writeError`・`writeErrorList`・`writeValidation` だけが作る(文字列を直接書くと、言語の切り替えをすり抜けるので、構造テストが禁じる)。**いま日本語になるのは、検証の失敗の 422、見つからない・権限・認証(サインインの失敗・確認トークン)・リクエストの形・写真・クエリの誤りの 4xx** で、Google のサインインの案内・OAuth の許可の画面・MCP のツールのエラーは、対応が済むまで英語のままである(済んだら、この項を更新する)。OAuth の RFC のプロトコルの識別子(`error` の値など)と、5xx の `internal server error` は、対応が済んでも英語のまま。
