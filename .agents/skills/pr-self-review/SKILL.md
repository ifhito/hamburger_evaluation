...
3. 以下だけを要約する:
   ...
   - 不足しているチェック、
   - 主要なレビューリスク。
4. **PR を ready にする前・人間にレビューを頼む前**は、このスキルに加えて、呼び出し元
   (`gh` を使える側)が **`/code-review`(Skill の `code-review`)を実行**する。
   `references/pr-self-review.md` の「`/code-review`」の節に従う。
...
4. `/code-review` を「実行した」と書いたのに、実際は呼んでいない(結果の要約も、未実行の理由も
   PR に残っていない)。
...
- [ ] (ready にする前)`/code-review` を実行し、結果の要約、または未実行の理由を PR のコメントに残した。
