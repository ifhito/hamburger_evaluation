...
- 数値・文字列・丸めの処理が**業務の規則**(件数・上限・文字数・期間・回数・状態・許す文字)なら、
  domain に置く。一覧のページ送りの規則(1 ページの既定の件数・上限・不正な値の丸め・`page` / `per_page`
  の解釈・続きがあるかを知るための件数の取り方)も同じで、`domain/page.go` にある。handler・usecase・
  adapter は、それを呼ぶだけで、数値や丸めを持たない。環境変数で決まる運用の設定(接続先・ポート・
  タイムアウト・ワーカーの数・キャッシュの大きさ)は、業務の規則ではないので、infra / cmd に置いてよい。
