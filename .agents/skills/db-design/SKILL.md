...
- **実運用に入る前は、追加のマイグレーションを作らず、最初の `CREATE TABLE`
  の定義を直す**(型の変更・制約の追加・列の追加。ユーザー決定)。残すデータが
  なく、変換や `DROP` の手順を持つ意味がないため。適用済みの DB は変わらない
  ので、作り直す(`migrate drop -f` → `migrate up` → `seed`)。実運用に
  入ったかどうかはユーザーが決める。入ったあとは、次の 2 項に従い、適用済みの
  マイグレーションを書き換えない。
- まず追加から(実運用に入ったあと): バックフィルと `NOT NULL`/制約の強化は
  カラム作成とは別ステップにする — 不可逆なマイグレーション 1 本にまとめない。
- 破壊的変更(カラム/テーブルの削除。実運用に入ったあと)は独立した
  マイグレーションにし、ユーザーの明示的な判断を得る。
