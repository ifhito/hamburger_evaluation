...
| # | 候補 | 無料枠 | 事前に必要なもの |
|---|---|---|---|
| 1 | Resend | 3,000 通/月、100 通/日 | メールアドレス。共有ドメインは自分宛にしか送れない |
| 2 | Mailjet | 6,000 通/月、200 通/日 | メールアドレス。送信者アドレスの認証のみ |
| 3 | Brevo | 300 通/日(無期限) | メールアドレス(フリーメール可) |
| 4 | MailerSend | 3,000 通/月 | メールアドレス。試用ドメインあり |

認証情報は `backend-go/.env.mail` に書く(`.env.mail.example` を参照)。
ホスト名とポートは各社のダッシュボードで確認すること。
...
### 計測でつまずいた点(2026-09-21 の実測より)

スクリプトを 3 回直した。いずれも**アプリではなく計測側の誤り**で、同じ罠は
他の Phase でも起きうる。

1. **同じ宛先では 2 回目以降が送られない。** アプリの冪等キーは確認待ちの受付 ID から
   作られるため、同じメールアドレスで繰り返すと送信も記録も発生しない。アプリは
   正しく動いている。**宛先を実行ごとに `+` エイリアスで一意にする**(`mail-verify.sh`
   が自動で行う)。ただし Resend の共有ドメインは `+` エイリアスを「自分のアドレス」と
   みなさないため、そこで弾かれる

2. **`mail_deliveries` の最新 1 行を無条件に読むと、直前の社の結果を拾う。**
   実行前の時刻を基準に取り、それより新しい行だけを見ること

3. **遮断の模擬にポート 2525 を使ってはいけない。** Mailjet など複数の事業者が
   2525 を正規の代替ポートとして受け付ける。確実に閉じている高位ポート(47777 など)を使う
...
4. **記録**: プロジェクト作成からデプロイ成功までの所要時間と操作回数。
5. **メモリの追試**: 512MB で写真の OOM が起きたら `--memory 1Gi` に上げて再測定し、
   無料枠(GB 秒)の消費がどれだけ増えるかを記録する。メモリを選べるのは 4 候補で
   Cloud Run だけなので、この追試もここだけで行う。

#### 4. Northflank

1. 登録し、GitHub を接続して `backend-go/Dockerfile.prod` を指定する。
2. **記録**: 無料枠の条件(サービス数・メモリ・常時起動かどうか)。
   条件が合わず使えない場合は、その事実を記録して Azure Container Apps に差し替える。
