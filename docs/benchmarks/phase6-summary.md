...
同じ `dist`(496KB、約 30 ファイル)を、同じ回線から同じ順序で配置したときの時間。
**コマンドが終わるまで**を計測している。

| ホスト | 方式 | コマンド終了まで | 反映まで |
|---|---|---|---|
| **Cloudflare Workers** | アップロード | **8.6 秒** | コマンド終了と同時 |
| Netlify | アップロード | 10.0 秒 | コマンド終了と同時 |
| Vercel | **ソースを送ってサーバーでビルド** | 25.6 秒 | コマンド終了と同時 |
| Render | **git push から自動ビルド** | push は 1 秒 | 約 1 分 |

参考: 手元のビルド(`pnpm run build`)は 3.6 秒。

### 「コマンドが終わった」と「反映された」は別の話

ここが最も引っかかった点である。**4 社で意味が 3 通りに分かれる。**

| 方式 | 何が起きているか | コマンド終了 = 反映? |
|---|---|---|
| Cloudflare・Netlify | 手元でビルドした成果物をアップロードする | **はい** |
| Vercel | ソースを固めて送り、**向こうでビルドする** | はい(ビルド完了まで待つ) |
| Render | git push が引き金。**ビルドは非同期** | **いいえ** |

Render だけは `git push` が 1 秒で戻る。そこから向こうでビルドが走るため、
**push の速さはデプロイの速さではない**。今回は push から約 1 分で反映された
(20 秒間隔の巡回で確認したため、精度は ±30 秒程度)。

反映の確認は、配信されている JS の中身を見に行くのが確実だった。

```bash
js=$(curl -s "${base}/" | grep -o 'assets/index-[A-Za-z0-9_-]*\.js' | head -1)
curl -s "${base}/${js}" | grep -c 'dedcd6'   # 今回変えた色が入っているか
```

### ハッシュは Vercel だけ一致しなかった

当初はバンドルのハッシュで照合しようとした。
結果は**3 社が手元のビルドと一致し、Vercel だけがずれた**。

| ホスト | ビルド場所 | ハッシュ | 手元と一致 |
|---|---|---|---|
| Cloudflare | 手元 | `index-mrO4dfYJ.js` | ○ |
| Netlify | 手元 | `index-mrO4dfYJ.js` | ○ |
| Render | **向こう** | `index-mrO4dfYJ.js` | ○ |
| Vercel | **向こう** | `index-CMyOLu0X.js` | **×** |

**向こうでビルドすること自体は原因ではない。** Render は向こうでビルドして一致している。
中身を比べても、埋め込まれた API の URL(`/api`)は同一だった。

| | バイト数 |
|---|---|
| 手元 / Cloudflare / Netlify / Render | 523,406 |
| Vercel | **524,397**(+991) |

差は約 1KB。環境変数ではなく、**Vercel 側の Node や依存の解決結果が手元と違う**
ことによる出力差だと考えられる(手元は Node 23.10.0)。

教訓は単純である。**ハッシュの一致は反映の判定に使えない。
変更した値そのものを探すのが確実である。**
...
**Netlify は同じ成果物でも 8 秒から 71 秒まで振れた。** 回線の状態ではなく、
向こうの処理待ちだと考えられる(アップロード量は毎回同じ)。

### 評価

日に何度も配置し直す個人開発では、**Cloudflare の安定した速さが効く**。
Vercel は毎回 20 秒台で、手元のビルド結果を捨てて向こうで作り直すぶん一貫して遅い。
ただし**手元の環境差がビルドに混入しないという利点**でもあり、一長一短である。
