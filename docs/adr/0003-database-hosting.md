...
### pgx で直接測った読み書き(200 回ずつ)

アプリの HTTP 層を挟まず測ると、次のようになった。

| 候補 | 往復のみ | 単発操作の幅 | トランザクション | 倍率 | 標準偏差 |
|---|---|---|---|---|---|
| Neon | 86.2ms | 86.3〜87.4ms | **347.9ms** | 4.04 倍 | 2.8ms |
| Supabase | 131.1ms | 131.3〜132.8ms | 526.8ms | 4.02 倍 | 14.6ms |
| Render | 146.2ms | 146.4〜147.8ms | 587.6ms | 4.02 倍 | 7.9ms |
| Xata | 177.4ms | 177.5〜180.0ms | 713.8ms | 4.02 倍 | 1.5ms |

分かったことは 3 つ。

1. **主キー 1 件も、3 テーブルの結合 20 件も、全件集計も、`SELECT 1` と区別がつかない。**
   DB の処理時間はネットワークに完全に埋もれる。性能指標で DB を選ぶのは誤り
2. **書き込みは距離のペナルティを 4 倍受ける。** レビューの作成は
   `BEGIN` → `INSERT reviews` → `INSERT 再計算依頼` → `COMMIT` の 4 往復で、
   4 社とも倍率が 4.02〜4.04 に一致した。往復回数が時間を決めている証拠である
3. **安定性は Neon と Xata が優れている**(標準偏差 2.8ms と 1.5ms)。
   Supabase は往復のみの最大値が p50 の 2.5 倍に達する外れ値を持つ
...
**Neon(無料枠。シンガポール `ap-southeast-1`)** を使う。

- 4 候補とも、マイグレーションが無改変で通った。**互換性では差がつかず、差は距離だけだった**(往復 p50: Neon 86ms、Supabase 131ms、Render 146ms、Xata 177ms)。
- Render Postgres は無料枠が 30 日で失効する。Supabase は 1 週間触らないと停止する。
- 分散 DB は採用しない。技術的な障害は減ったが、利点が要件にない。

有料化するとき: Neon の Launch(従量。月額の最低なし)に上げる。別の事業者へは `pg_dump` / `pg_restore` で移す。

## 結果・影響

- **接続 URL を 2 つ使い分ける。** アプリ(Cloud Run)は pooler 経由の URL、マイグレーション(CD)は直結の URL。pooler 経由だとマイグレーションが `unnamed prepared statement does not exist` で落ちることがある(検証中に 2 回発生)。
- **マイグレーションは CD が当てる**(golang-migrate `v4.18.1`)。新しいコードを出す前に当てるので、変更は前方互換にする。列の削除や名前の変更は 2 回のリリースに分ける。
- **適用済みのマイグレーションを書き換えない。** 検証中、main で最初の `CREATE TABLE` が書き換えられ、Neon のスキーマと食い違って Google ログインが落ちた(`password_digest` の NOT NULL)。実運用前だったので DB を作り直して解決したが、今後は新しいマイグレーションで直す。
- Neon の休止からの復帰は速いが、最初の 1 クエリだけ遅い。`GET /up` が DB に ping するので、ヘルスチェックが実質的なウォームアップになる。
- **期限切れ行の削除ジョブが要る**(`oauth_token_sessions`、`mail_deliveries`)。未実装。[ADR-0005](0005-async-job-platform.md) で扱う。
- 無料枠のバックアップ保持は短い。重要データが増えたら、週次の `pg_dump` を R2([ADR-0004](0004-photo-storage.md))に置く。
