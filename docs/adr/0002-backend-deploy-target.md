...
| Northflank | 無期限(条件は要確認) | 要確認 | ○ **常時起動** | 要確認 | 要確認 | ○ |
...
| # | 候補 | 起動モデル | 立地 |
|---|---|---|---|
| 1 | Render | 15 分でスリープ | シンガポール |
| 2 | Zeabur | スリープあり | 東京 |
| 3 | Cloud Run | scale-to-zero | 東京 |
| 4 | Northflank | 常時起動 | 要確認 |

メモリは候補ではなく**各ホストの属性**として記録する。無料枠でいくつか、引き上げられるか、
引き上げるといくらかを全ホストで測る。メモリを自由に選べるのは Cloud Run だけである。
