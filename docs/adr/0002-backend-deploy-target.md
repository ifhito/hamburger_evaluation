...
検証では次の 4 つを測る。**起動モデルがすべて違うものを選んだ。**

| # | 候補 | 種別 | カード | 起動モデル | 立地 |
|---|---|---|---|---|---|
| 1 | Render | **無料プラン** | 不要 | 15 分でスリープ | シンガポール |
| 2 | Back4App Containers | **無料プラン** | 不要 | 常時起動(256MB) | 要確認 |
| 3 | Cloud Run | **無料枠** | 要 | scale-to-zero | 東京 |
| 4 | Northflank(Sandbox) | 無料枠 | 検証のみ | 常時起動 | Asia East |

メモリは候補ではなく**各ホストの属性**として記録する。メモリを自由に選べるのは Cloud Run だけである。

**「無料プラン」と「無料枠」を区別する。** 前者は上限に達すると止まるだけで請求されない。
後者は有料サービスの一部で、超えた分は課金される。個人開発では**事故で請求が発生しうるか**が
重い判断材料になる。Cloud Run の予算アラートは通知するだけで支払いを止めない。

### 候補から外したもの(2026-09-21〜22 に確認)

| 候補 | 理由 |
|---|---|
| Koyeb | 利用登録ができなかった。東京で API・DB・静的を同居できる数少ない無料 PaaS だっただけに惜しい |
| Zeabur | 有料だった |
| Fly.io | **無料枠は存在しない。** トライアルは「2 時間の稼働または 7 日」のいずれか早い方で、放置を伴う計測に使えない(公式ドキュメントで確認) |
| Azure Container Apps | 無料枠はあるが Cloud Run と起動モデルが重なる |

**「無料 + 東京 + PaaS」という組み合わせは、事実上 Cloud Run しか残らなかった。**
これ自体が本 ADR の結論の 1 つである。
