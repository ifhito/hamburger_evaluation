...
|---|---|---|---|---|---|---|
| Render | 無期限。750 時間/月。15 分でスリープ。カード不要 | 512MB | ○ プロセスは生きている | 要確認 | シンガポール | ◎ Pre-Deploy Command |
| Zeabur | 無料枠あり(要確認) | 要確認 | ○ | 要確認 | ◎ 東京 | ○ |
| Google Cloud Run | 無期限。200 万リクエスト/月、36 万 GB 秒/月。カード要 | **設定可**(既定 512MB) | **△ 既定でリクエスト外は絞られる** | ✕ 25/587 を塞ぐ | ◎ 東京 | ○ Jobs |
| Koyeb | 無期限。1 サービス。カード要 | 512MB | ○ | 要確認 | ◎ 東京 | ○ |
...
**Render** を第一候補、**Zeabur** を東京優先の代替とする。Cloud Run は第 3 位。

ただし**メモリ問題が未解決のため、いずれも暫定である**。512MB のままでは写真の同時投稿で落ちる。
取りうる手は 4 つで、Phase 4 の実測で選ぶ。

| # | 手 | 費用 | 副作用 |
|---|---|---|---|
| 1 | 768MB 以上の有料プランにする | 月額が発生 | なし |
| 2 | **写真処理をセマフォで直列化する** | 無料 | 同時投稿時のレスポンスが遅くなる |
| 3 | 受け付ける画素数の上限を下げる | 無料 | 大きな写真が 422 になる |
| 4 | Cloud Run で 1GB に設定する | 従量(低トラフィックならほぼ無料) | CPU 抑制と SMTP の懸念が残る |

2 は数行で実装でき、無料枠を維持したいなら最も安い。

## 結果・影響

- 本番用 Dockerfile は作成済み(`backend-go/Dockerfile.prod`、29MB)。
- マイグレーションの適用方法を決める(Pre-Deploy Command で CLI を叩くか、起動時適用か)。
  複数インスタンスで同時起動しても壊れないよう、起動時適用にするなら advisory lock を使うこと。
- `GET /up` をヘルスチェックに設定する。
- graceful shutdown を確認する。ワーカーの `Stop` が処理中のバッチを待つ実装になっている。
- `DB_MAX_CONNS` を DB 側の上限に合わせる([ADR-0003](0003-database-hosting.md))。
- **OOM はログに何も残らない**(カーネルによる強制終了)。監視は `ExitCode=137` と再起動回数で行う。
- 残る計測項目: リクエスト外でのワーカー動作、外向き SMTP の疎通。いずれも Phase 4 で測る。
