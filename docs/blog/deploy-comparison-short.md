...
東京リージョンからの応答が速く、SMTP に接続でき、原寸の写真を送っても処理できました。そのうえ今回の利用量であれば無料枠に収まりそうだったため、Cloud Run を選びました。ほかの3社も API 自体は動き、ブラウザ側で縮小した写真であれば扱えます。ただ、Render は SMTP に接続できず、Back4App と Northflank は原寸写真の処理に必要なメモリが足りませんでした。
...
|---|---|---|
| API | Cloud Run(東京、1GB) | 応答が速く、SMTP と原寸写真の処理が通り、今回の利用量なら無料枠に収まりそうでした |
