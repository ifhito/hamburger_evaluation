...
import { oauthApi, oauthApiClient } from "./oauthApiClient";
...
describe("oauthApi", () => {
  ...
  it("listApps は GET /oauth/grants を呼び、キーを camelCase にして一覧を返す", async () => {
    respondWith(200, [
      {
        id: "g1",
        client_id: "app",
    ...
    const res = await oauthApi.listApps();

    expect(sent.method).toBe("get");
    expect(sent.url).toBe("/oauth/grants");
    expect(res).toEqual([
      {
        id: "g1",
        clientId: "app",
